...
$id_programa = isset($_SESSION['id_programa']) ? $_SESSION['id_programa'] : "" ;//$_SESSION['id_programa'];
...
$planesEstudios = [];
if ($id_programa) {
    $queryPlanEstudios = "SELECT idplan, codigo_plan, anio FROM vista_planestudios_programaestudios WHERE id_programa = '$id_programa'";
    $resultPlanEstudios = mysqli_query($conexion, $queryPlanEstudios);
    
    while ($row = mysqli_fetch_assoc($resultPlanEstudios)) {
        $planesEstudios[] = $row;
    }
}
?>
                ...
                <div class="col-md-12">
                    <div class="card card-primary" style="box-shadow: 0px 5px 5px 5px #c0c0c0">
                        ...
                        <div class="text-center">
                           <strong> GENERAR MATRICULA</strong>  <!--<span id="message" style="color: green;"><?php echo $message; ?></span>-->
                           
                        </div>
                        </div>
                        <div class="card-body">
                        <form action="controladormatricula.php" method="POST" enctype="multipart/form-data" name="formbuscar">
                                <div class="row">
                            ...
                            <form action="controladormatricula.php" method="POST" id="formulario_matricula" name="formulario_matricula" enctype="multipart/form-data">
                                <div class="row">
                               ...
                               <div class="col-md-3">
                                    ...
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label for="">Plan de Estudios</label> <span style="color: red;">(*)</span>
                                            <select name="plan_estudios" id="plan_estudios" class="form-control" required>
											    <option value="" >Seleccione</option>
												<?php foreach ($planesEstudios as $plan): ?>
                                                    <option value="<?= $plan['idplan'] ?>"><?= $plan['codigo_plan'] . " - " . $plan['anio'] ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
                                 </div>
